...
class InternControllerTest extends UnitSuite with ScalatraSuite with TestEnvironment {
  ...
  val DefaultApiImageMetaInformation = api.ImageMetaInformation("1", s"${ImageApiProperties.ImageApiUrlBase}1", List(), List(), "https://images.local.api.digitallibrary.io/test.jpg", 0, "", api.Copyright(api.License("", "", None), "", List()), List(), List())
