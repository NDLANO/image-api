...
class V12__AddImageMetadataTest extends UnitSuite with TestEnvironment {
  val migration = new V12__AddImageMetadata
  ...
  test("migration should update to new status format") {
    ...
    when(imageStreamMock.sourceImage).thenReturn(bufferedImageMock)
  ...
  test("migration not do anything if the document already has new status format") {
    when(imageStorage.get(testUrl)).thenReturn(Success(imageStreamMock))
    when(imageStreamMock.sourceImage).thenReturn(bufferedImageMock)
    when(bufferedImageMock.getWidth()).thenReturn(100)
    when(bufferedImageMock.getHeight()).thenReturn(200)
    val original =
      s"""{"copyright":{"license":"COPYRIGHTED","creators":[{"name":"henrik","type":"Writer"}],"processors":[],"rightsholders":[]},"title":[{"title":"tittel","language":"nb","width":50,"height":50}]}"""
