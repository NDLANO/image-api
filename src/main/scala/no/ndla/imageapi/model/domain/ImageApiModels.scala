...
case class ImageMetaInformation(id: Option[Long], titles: Seq[ImageTitle], alttexts: Seq[ImageAltText], url: String, size: Int, contentType: String, copyright: Copyright, tags: Seq[ImageTag], captions: Seq[ImageCaption])
...
object ImageMetaInformation extends SQLSyntaxSupport[ImageMetaInformation] {
  ...
  def apply(im: ResultName[ImageMetaInformation])(rs: WrappedResultSet): ImageMetaInformation = {
    ...
    ImageMetaInformation(Some(rs.long(im.c("id"))), meta.titles, meta.alttexts, meta.url, meta.size, meta.contentType , meta.copyright, meta.tags, meta.captions)
  }
